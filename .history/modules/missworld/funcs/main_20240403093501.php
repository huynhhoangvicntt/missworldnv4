...
while ($row = $query->fetch()){
    ...
    $sql = 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_missworld WHERE name = ' . $name . ' ORDER BY id ASC';
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // Hiển thị số lượng bình chọn cho mỗi mục
    while($row = $result->fetch_assoc()) {
        echo "Option " . $row["id"] . ": " . $row["vote_count"] . " votes<br>";
    }
} else {
