    ...
    if (empty($error)) {
        if (!$id) {
            ...
            $sql = "INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . "_onecat2s (
                title, image, keywords, description, weight, add_time, edit_time
            ) VALUES (
                :title, :image, :keywords, :description, " . $weight . ", " . NV_CURRENTTIME . ", 0
            )";
        } else {
            $sql = "UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_onecat2s SET
                title = :title, image = :image, keywords = :keywords,
