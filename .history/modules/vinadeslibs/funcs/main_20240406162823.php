...
$array = [];
$query = $db->query("SELECT * FROM " . NV_PREFIXLANG . "_vinadeslibs_onecat2s");
while ($row = $query->fetch()){
    $array_data[$row["id"]] = $row;
   
    $vote = $_POST["vote"];
    // Tăng số lượng bình chọn cho lựa chọn đã chọn trong cơ sở dữ liệu
    $sql = "UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_onecat2s SET voted=" . ++$voted . " WHERE id=" . $row['id'];
        $db->query($sql);
}
